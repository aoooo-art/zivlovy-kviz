...
document.getElementById('quiz-form').addEventListener('submit', function (e) {
  ...
  const redirectMap = {
    'země': 'https://www.aoooo.art/zivel-zeme',
    'voda': 'https://www.aoooo.art/zivel-voda',
    'vzduch': 'https://www.aoooo.art/zivel-vzduch',
    'oheň': 'https://www.aoooo.art/zivel-ohen',
    'duch': 'https://www.aoooo.art/zivel-duch'
  };
  ...
  if (topElements.length === 1) {
    ...
    top.location.href = redirectMap[zivel];
    return;
  }
  ...
  rozstrel.innerHTML = `
    <fieldset style="margin-top: 40px; border-top: 1px dotted #999; padding-top: 20px;">
      <legend style="font-weight: bold;">Rozhodující otázka: Cítíš se spíš jako…</legend>
      <label><input type="radio" name="rozstrel" value="${z1}"> ${otázky[0]}</label><br>
      <label><input type="radio" name="rozstrel" value="${z2}"> ${otázky[1]}</label><br>
  ...
  document.getElementById('rozstrel-submit').addEventListener('click', function () {
    ...
    if (val) {
      top.location.href = redirectMap[val];
    }
  });
});
